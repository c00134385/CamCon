...
SYS_TICK_TYPE systick;

int SysTick_Start(void)
{
	uint32_t ticks = 10*fac_ms;
...
void SysTick_HandlerService(void)
{
    systick.TenMs++;
    if(systick.TenMs%100==0)
        systick.Sec++;
...
void Wait10Ms(int i)
{
			unsigned long end_t = GetSysTick_10Ms()+i;
