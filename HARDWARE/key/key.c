...
{ 
	...
	GPIO_InitStructure.GPIO_Pin  = GPIO_Pin_1|GPIO_Pin_0|GPIO_Pin_8;//PA0 1 8
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU; //设置成上拉输入
 	GPIO_Init(GPIOA, &GPIO_InitStructure);//初始化GPIOa0,1,8

    //初始化KEY_F6-->GPIOB.5,KEY_F7-->GPIOB.8   KEY_F8-->GPIOB.9上拉输入
    ///KEY_U-->GPIOB.12 KEY_D-->GPIOB.13 KEY_L-->GPIOB.14 KEY_R-->GPIOB.15
	GPIO_InitStructure.GPIO_Pin  = GPIO_Pin_5|GPIO_Pin_8|GPIO_Pin_9|GPIO_Pin_12|GPIO_Pin_13|GPIO_Pin_14|GPIO_Pin_15;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU; //设置成上拉输入
 	GPIO_Init(GPIOB, &GPIO_InitStructure);//初始化GPIOB


	//初始化KEY_F1-->GPIOC13.5,KEY_F2-->GPIOC.14   KEY_F3-->GPIOC.15上拉输入
...
#define GENNUM_3G_ENABLE PBout(4)// PE5
void gennum_3g_enable(BOOL on)
{
    printf("\r\n %s() is called. enable:%d", __FUNCTION__, on);
    if(on)
    {
        GENNUM_3G_ENABLE = 1;  // PB4 high level
    }
    else
    {
        GENNUM_3G_ENABLE = 0;  // PB4 low level
    }
}

#define VIDEO_ENABLE PBout(1)// PB1
